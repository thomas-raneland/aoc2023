...
const realinput = `L77FF.FJF|FL-7F7FF-F7F7FL.|.F-77.F--|.J7.FFLL--|7-7.7-L77F7-7.F-7F7-FF7F|77.|-JF7-FFF.L-7FFF7---|F7.|.FLF-JF-|-7---|F77FF-F-77FL77|-77.|-J.F
...
for (input of [testinput, realinput]) {
  const [ sketch, start ] = parse(input);
  const loop = findLoop(sketch, start);
  console.log("Part I: " + Math.ceil(loop.length / 2));

  sketch.set(start, "."); // FIXME: replacement tile should be computed from neighbors
  console.log("Part II: " + findArea(sketch, loop));
}

function findLoop(sketch, start) {
  const loop = [];
  let pos = start;
  var back;
  ...
  do {
    for (forward of [..."NESW"]) {
      if (forward != back && hasExit(sketch.get(pos), forward)) {
        const nextPos = move(pos, forward);

        if (sketch.get(nextPos) && hasExit(sketch.get(nextPos), opposite(forward))) {
          loop.push(pos);
          pos = nextPos;
...
function findArea(sketch, loop) {
  var loopPositions = new Set();
  loop.forEach(p => loopPositions.add(p));

  return Array.from(sketch.keys())
    .filter(pos => !loopPositions.has(pos))
    .filter(pos => crossingsNorth(sketch, loopPositions, pos) % 2 == 1)
    .length;
}

function crossingsNorth(sketch, loop, pos) {
    let count = 0;

    while (fromPos(pos).y > 0) {
        pos = move(pos, "N");
        const tile = sketch.get(pos);

        if (loop.has(pos) && hasExit(tile, "E")) {
            count++;
        }
    }
...
function hasExit(tile, dir) {
  switch (tile) {
    case '.': return false;
    case 'S': return true;
    case '|': return dir === 'N' || dir ==='S';
    case '-': return dir === 'E' || dir ==='W';
    case 'L': return dir === 'N' || dir ==='E';
    case 'J': return dir === 'N' || dir ==='W';
    case '7': return dir === 'S' || dir ==='W';
    case 'F': return dir === 'S' || dir ==='E';
  };
  throw new Error();
}
...
function opposite(dir) {
  const mapping = "NSNWEW";
  return mapping[mapping.indexOf(dir) + 1];
}
